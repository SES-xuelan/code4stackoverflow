...
--
local log = { debug = print }
local moves_limited = 2
local moves_txt
local update_moves
local check_outofmoves
...
function create()
    ...
    moves_txt = display.newText(layers.interface, "Moves:" .. tostring(moves_limited), 0, 0, gameFont, 24)
    moves_txt.x = centerX
    moves_txt.y = h - 30
    moves_txt:setFillColor(51 / 255, 16 / 255, 95 / 255)
...
testForMatches = function(lastMatches)
    ...
    if (foundMatch) then
        log.debug("update_moves lastMatches", lastMatches)
        if lastMatches == 0 then
            update_moves()
        end
        ...
        timer.performWithDelay(250, function()
            if check_outofmoves() then
                touchesAllowed = false
                log.debug("OUT OF MOVES!!!")
                -- add your own code for out of moves
            else
                touchesAllowed = true
            end
        end)
...
update_moves = function()
    moves_limited = moves_limited - 1
    moves_txt.text = "Moves:" .. tostring(moves_limited)
end

check_outofmoves = function()
    return moves_limited == 0
end
